...
namespace Xiao
{
	...
	/// </summary>
    public class S_XiaoRare_EmotionalTurbulence : Skill_Extended, IP_DamageChange_sumoperation
    {
        ...
        {
            if (BChar.EmotionLevel() >= 1 && SkillD.MySkill.KeyID == ModItemKeys.Skill_S_XiaoRare_EmotionalTurbulence)
            {
                PlusDamage += (int)(this.BChar.GetStat.atk * 0.4f) * BChar.EmotionLevel();
            }

            else if (BChar.EmotionLevel() >= 5 && SkillD.MySkill.KeyID == ModItemKeys.Skill_S_XiaoRare_EmotionalTurbulence)
            {
