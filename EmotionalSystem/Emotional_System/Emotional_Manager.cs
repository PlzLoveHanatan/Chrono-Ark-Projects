...
namespace EmotionalSystem
{
	public static class EmotionalManager
	{
		...
		public static void GetEmotion(this BattleChar bc, Vector3? source, bool isPos)
		{
			...
			if (Utils.EmotionalCoinPaticles && source != null)
			{
				GameObject prefab;
				if (isPos)
				{
		...
		public static CharEmotion MyEmotion(this BattleChar bc)
		{
			if (bc is BattleAlly)
			{
				var buff = bc.BuffReturn(ModItemKeys.Buff_B_Ally_Emotional_Level) as EmotionalLevelAlly;
				return buff?.Emotion;
			}
			else if (bc is BattleEnemy)
			{
				var buff = bc.BuffReturn(ModItemKeys.Buff_B_Enemy_Emotional_Level) as EmotionalLevelEnemy;
				return buff?.Emotion;
			}
			return null;
		}

		public static void SetEmotionCapAlly(this BattleChar bc, bool isEmotionCap = false)
		{
		...
		public static void SetEmotionCapEnemy(this BattleChar bc, bool isEmotionCap = true)
		{
			if (Utils.ReturnBuff(bc, ModItemKeys.Buff_B_Enemy_Emotional_Level) is EmotionalLevelEnemy buff)
			{
		...
		public static void AddEmotionLevel(this BattleChar bc, bool isForceLevelUp = false)
		{
			...
			int coinsNeeded = EmotionalLevelAlly.CoinsToLevelUp[currentLevel];
